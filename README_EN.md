...
<b>A native AI PPT generation application based on nano banana pro🍌<br></b>
<b>From idea to presentation in minutes—no tedious formatting, just verbal revisions. Move towards a true "Vibe PPT".</b>
...
Have you ever found yourself in this dilemma: the presentation is due tomorrow, but the slides are still blank; your head is filled with brilliant ideas, but your enthusiasm is drained by tedious layout and design?

We long to quickly create presentations that are both professional and well-designed. While traditional AI PPT generators generally satisfy the need for "speed," they still suffer from the following issues:
...
- 3️⃣ The output looks similar, with severe homogenization.
- 4️⃣ Low-quality assets that lack relevance.
- 5️⃣ Disjointed text-image layouts with a poor sense of design.

These shortcomings make it difficult for traditional AI PPT generators to simultaneously meet our two core needs: "speed" and "aesthetics." Even those claiming to be "Vibe PPT" are, in my view, far from having enough "Vibe."

However, the emergence of the nano banana 🍌 model has changed everything. I tried using 🍌pro to generate PPT pages and found that the results were exceptional in terms of quality, aesthetics, and consistency. It accurately renders almost all text required by the prompt and follows the style of reference images precisely. So, why not build a native "Vibe PPT" application based on 🍌pro?

## 👨‍💻 Applicable Scenarios

1. **Newbies**: Quickly generate beautiful PPTs with zero entry barrier and no design experience, reducing the hassle of choosing templates.
2. **PPT Professionals**: Quickly gain design inspiration by referencing AI-generated layouts and combinations of graphic and text elements.
3. **Educators**: Rapidly convert teaching content into illustrated lesson plan PPTs to enhance classroom effectiveness.
4. **Students**: Quickly complete presentation assignments, allowing them to focus on content rather than layout and formatting.
5. **Office Professionals**: Quickly visualize business proposals and product introductions with rapid adaptation to various scenarios.

<p>
  <b>🎯 Goal: Lower the entry barrier for PPT creation, enabling everyone to quickly create aesthetic and professional presentations.</b>
</p>
...
| **R&D and Industrialization of Intelligent Production Line Equipment for Pre-prepared Dishes** | **The Evolution of Money: A Journey from Shells to Banknotes** |
...
- **Intelligent Extraction**: Automatically identify key points, image links, and chart information within the text to provide rich assets for generation.
...
No longer limited by complex menu buttons, issue editing commands directly through **natural language**.
- **Local In-painting**: Make conversational modifications to specific areas (e.g., "change this chart to a pie chart").
...
- [2026-04-25]: Asset Toolbox launched — Added three new modes: full-image editing, selection editing (overlay/replace), and smart erase, providing a unified one-stop operation.
- [2026-04-25]: Supports account binding via OpenAI official OAuth. Once bound, Codex can be used directly as a text/image generation provider without manually entering an API Key (based on the official OpenAI OAuth PKCE authorization flow, non-reverse engineered).
...
- [2026-04-23]: Added support for the gpt-image-2 model. The editable background export effect has also been improved due to enhanced model capabilities (Select "Generative Acquisition" in Settings -> Export Options -> Background Acquisition).
...
- [2026-02-09]: New Features and Optimizations
  * New Features
    * Supports pasting images into the home page, outlines, and description cards for immediate recognition, with an improved interactive experience.
  ...
  * Fixes and Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, incorrect task polling projects, infinite polling for description generation, image preview memory leaks, and handling of partial batch deletion failures.
...
- [2026-01-04]: v0.4.0 Released: Comprehensive upgrade for editable PPTX export:
  ...
  * Optimized the export workflow, significantly reducing leftover text on background images after export;
  * Supports multi-page selection logic for flexible generation and export of specific pages.
  * **See https://github.com/Anionex/banana-slides/issues/121 for detailed effects and usage.**

- [2025-12-27]: Added support for imageless template mode and high-quality text presets. PPT page styles can now be controlled via pure text descriptions.
...
| ✅ Completed | Area selection and Vibe voice editing for individual slides |
...
1. One-click deploy and start this application via RainYun (High bandwidth, suitable for HD image generation and downloads. New users get a 15-day free trial)
...
If you are using **Windows or macOS**, please [install **Docker Desktop**](https://docs.docker.com/desktop/setup/install/windows-install/) first and ensure Docker is running (check the system tray icon for Windows; the menu bar icon for macOS), then follow the same steps in the documentation.
...
**(Optional, can also be configured in the UI after startup)** Edit the `.env` file to configure the necessary environment variables:
> **The LLM interfaces in this project follow the AIHubMix platform standard. It is recommended to use [AIHubMix (click here to access directly)](https://aihubmix.com/?aff=17EC) to obtain an API key and reduce migration costs.**<br>
> **Friendly Tip: The Google nano banana pro model interface costs are relatively high; please be mindful of the invocation costs.**
```env
...
**Use the new editable export configuration method for better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (which provides a sufficient free usage quota). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for more details.
...
1. Go to [GCP Console](https://console.cloud.google.com/), create a service account and download the JSON key file.
...
3. Set in `.env`:
   ...
   VERTEX_LOCATION=global
   ```
4. If deploying with Docker, you also need to uncomment relevant sections in `docker-compose.yml`, mount the key file to the container, and set the `GOOGLE_APPLICATION_CREDENTIALS` environment variable.
...
# Launching with Pre-built Images (No Build Required)
...
> **The LLM API in this project follows the AIHubMix platform format. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain your API key to reduce migration costs.** 
```env
...
│   │   │   ├── Home.tsx        # Home page (Project creation)
...
1. **The generated page text has garbled characters or is not clear**
    - You can choose a higher resolution output (the OpenAI format may not support increasing resolution; it is recommended to use the Gemini format). Based on tests, adjusting the resolution from 1k to 2k before generating the page will significantly improve text rendering quality.
...
2. **Poor results when exporting editable PPT, such as overlapping text or missing styles**
    - In 90% of cases, this is due to an API configuration issue. You can refer to the troubleshooting and solutions in [issue 121](https://github.com/Anionex/banana-slides/issues/121).
...
4. **Receiving a 503 error or Retry Error during content generation**
    - You can check the Docker backend logs using the commands in the README to locate the detailed error for the 503 issue. It is generally caused by incorrect model configuration.

5. **Why doesn't the API Key take effect after setting it in .env?**
    - You need to restart the Docker container to apply changes after editing `.env` during runtime.
...
It can be freely used for non-commercial purposes such as personal study, research, testing, education, or non-profit scientific research;
<details> 
<summary> Details </summary>
A Commercial License is required for commercial use (e.g., closed-source usage, private deployment and delivery, integrating this project into closed-source products, or providing services without disclosing the corresponding source code). Please contact the author: davidyang042@gmail.com
- Contact: davidyang042@gmail.com
</details>
...
<details>
  ...
  "Aggregating global multi-model API service providers. Enjoy secure, stable services with access to the world's latest models at a lower price."
</details>
