...
<b> Automatically extract charts from attachments, upload any materials, and make modifications via voice/text prompts—moving towards a true "Vibe PPT" experience. </b>
...
Have you ever found yourself in this dilemma: you have a presentation tomorrow, but your slides are still blank; your head is full of brilliant ideas, but your enthusiasm is drained by tedious layout and design?
...
These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two core requirements: "speed" and "beauty." Even those claiming to be "Vibe PPT" are, in my eyes, still far from having a true "vibe."
...
- **Natural Language Editing**: Supports modifying outlines or descriptions using natural language (e.g., "Change the third page to a case study"), with real-time AI adjustments.
...
- **Multi-format Support**: Upload PDF/Docx/MD/Txt files, and the system will automatically parse the content.
- **Intelligent Extraction**: Automatically identifies key points, image links, and chart information within the text to provide rich material for generation.
...
- **Partial Redraw**: Make conversational edits to specific areas (e.g., "Change this chart to a pie chart").
...
- **Perfect Compatibility**: Default 16:9 aspect ratio with no manual layout adjustments required, ready for direct presentation.
...
| Post-editing | Not supported | **Selection editing + Prompt-based editing** |
...
- 【2-9】：
  * New Features
    * Support for pasting images on the homepage, outline, and description cards with instant recognition and an improved interactive experience.
  ...
  * Fixes and Experience Optimizations
    * Fixed export-related 500 errors, reference file association timing, outline/page data misalignment, task polling errors, infinite polling in description generation, image preview memory leaks, and partial failure handling in batch deletions.
...
- 【1-4】 : v0.4.0 Release: Comprehensive upgrade for editable pptx exports:
  * Maximum restoration of text styles from images, including font size, color, bolding, etc.
...
| ✅ Completed | Assets module: asset generation, uploading, etc. |
...
| 🔄 In Progress | Support editable .pptx export with multi-layering and precise cutouts |
...
| 🏢 Pro Version | User system |

## 📦 Usage

### (New) One-click deployment using application templates

This is the easiest method. No Docker installation or project download is required. You can access the application directly after creation.

1. One-click deployment and startup of this application via Rainyun (New users get a 15-day free trial + double credit on first top-up policy)
...
If you are using **Windows or macOS**, please install **Docker Desktop** first and ensure Docker is running (check the system tray icon on Windows or the menu bar icon on macOS), then follow the same steps as described in the documentation.
...
> **The Large Language Model (LLM) interfaces in this project follow the AIHubMix platform format standard. It is recommended to use [AIHubMix](https://aihubmix.com/?aff=17EC) to obtain API keys to reduce migration costs.**<br>
...
# API Keys for Various Providers (Only configure the providers you want to use)
...
**Use the new editable export configuration method to achieve better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter), and fill it in the `BAIDU_API_KEY` field of the `.env` file (there is a generous free usage quota). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.


<details>
  <summary>📒 Vertex AI Configuration Guide (for GCP users)</summary>

Google Cloud Vertex AI allows Gemini models to be called via GCP service accounts. New users can use promotional credits. Configuration steps:
...
> The `gemini-3-*` series of models require `VERTEX_LOCATION=global`
...
We have created this WeChat group to facilitate communication and mutual assistance.

Suggestions for new features or feedback are welcome. I will also answer your questions ~~casually~~.

<img width="312" alt="image" src="https://github.com/user-attachments/assets/3e0f9fd9-7613-46bc-a37b-17211d40f5fc" />

## **🔧 Frequently Asked Questions**

1. **Garbled text or blurry text in generated pages**
    - You can choose a higher resolution output (the OpenAI format may not support increasing resolution; Gemini format is recommended). Testing shows that increasing resolution from 1k to 2k before generation significantly improves text rendering quality.
    - Ensure the page description includes the specific text content you want to render.

2. **Poor results when exporting editable PPT, such as overlapping text or missing styles**
    - In 90% of cases, this is due to API configuration issues. Please refer to [issue 121](https://github.com/Anionex/banana-slides/issues/121) for troubleshooting and solutions.
...
4. **Encountering 503 error or Retry Error when generating content**
...
5. **Why is the API Key set in .env not taking effect?**
    - You need to restart the Docker container after editing `.env` to apply the changes.
    - If you have previously configured parameters on the web settings page, they will override the `.env` settings. You can revert to the `.env` settings by clicking "Restore Default Settings."
...
A Commercial License is required (e.g., if you wish to use it in a closed-source manner, deliver private deployments, integrate this project into closed-source products, or provide services without disclosing the corresponding source code). Please contact the author: anionex@qq.com
...
<details>
  ...
  "An aggregator of global multi-model API service providers. Enjoy secure, stable services with access to the world's latest models within 72 hours at lower prices."
</details>
