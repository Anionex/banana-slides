...
<p>
  ...
  <b>Go from idea to presentation in minutes—no tedious formatting, just conversational edits. Step into true "Vibe PPT".</b>
</p>
<p>
...
<td valign="middle">Thanks to <a href="https://aihubmix.com/?aff=17EC">AIHubMix</a> for sponsoring this project! AIHubMix is a stable, high-concurrency AI large model API aggregation platform. A single API Key provides access to mainstream models like Claude, GPT, Gemini, and DeepSeek, compatible with multiple protocols. When registering, overseas users please use the <a href="https://aihubmix.com/?aff=17EC">AIHubMix portal</a>, and Chinese mainland users please use the <a href="https://inferera.com/?aff=17EC">Inferera portal</a>.</td>
</tr>
<tr>
...
- **[2026-09-05]**: 0.9.0 Release Candidate 7 released, fixing an issue where Codex (OpenAI OAuth) was connected but image generation returned a 400 error: Internal call for the main model updated from `gpt-5.4` to `gpt-5.6-terra`, while the image model remains `gpt-image-2` (no need to change the image model to a text model). This version also includes content-driven style descriptions and SenseNova U1 image generation support; [View Download and Installation Instructions](https://github.com/Anionex/banana-slides/releases/tag/v0.9.0-rc.7)
- **[2026-08-30]**: 0.9.0 Release Candidate 6 released, adding a togglable update check on desktop startup, update cards with summary and full log links, as well as download progress, failure retry, and restart to install; also fixed APIMart OpenAI-compatible asynchronous image tasks, non-streaming requests, and 1K/2K/4K resolution passing; [View Download and Installation Instructions](https://github.com/Anionex/banana-slides/releases/tag/v0.9.0-rc.6)
- **[2026-08-29]**: 0.9.0 Release Candidate 5 released, adding an immersive online slide player and APIMart OpenAI-compatible Provider presets; desktop update checks now correctly follow the RC channel; improved MinerU credential error prompts for PPT transformation, fixed SSRF risks for remote images in reference documents, and set image editing to enter marquee selection mode by default; [View Download and Installation Instructions](https://github.com/Anionex/banana-slides/releases/tag/v0.9.0-rc.5)
- **[2026-08-20]**: 0.9.0 Release Candidate 4 released, focusing on fixing unavailable LazyLLM online providers (qwen, etc.) and missing SOCKS proxy dependencies in the desktop packaged version; restored the "Previous Step" button on the preview page to return to the description editing page, fixed export task popup occlusion, and desktop property drawer interaction; [One-click Download and Install](https://github.com/Anionex/banana-slides/releases/tag/v0.9.0-rc.4)
- **[2026-08-20]**: Restored "Previous Step" button on the preview page, allowing one-click return from slide preview to the description editing page for further modifications.
- **[2026-08-20]**: Fixed an issue where the export task popup was blocked by the page property drawer; the desktop page property drawer now expands by default and automatically adapts to the window width.
...
- **[2026-07-15]**: Custom outline/description requirement presets now automatically repair corrupted browser cache, preserving valid presets to prevent abnormal cache from blocking the editing page.
- **[2026-07-11]**: 0.9.0 Release Candidate 2 released, including all capabilities of RC1 and fixing MinerU directory inconsistency and FFprobe path errors for explainer videos on Windows desktop; [One-click Download and Install](https://github.com/Anionex/banana-slides/releases/tag/v0.9.0-rc.2)
- **[2026-06-23]**: Page-by-page templates launched — supports both unified template and independent template per page modes. Users can upload images or PDFs to build a project template library. AI automatically analyzes template styles and intelligently matches each page with one click, or allows manual binding per page. Both modes support two-way switching at any time ([Documentation](https://docs.bananaslides.online/zh/features/templates))
- **[2026-04-25]**: Asset Toolbox launched — adds three new modes: full image editing, marquee editing (overlay/replace), and smart erase, based on existing asset generation, providing a unified entry for one-stop operations.
- **[2026-04-25]**: Supports OpenAI official OAuth login and account binding. Once bound, Codex can be used directly as the text/image generation provider without manually filling in the API Key. Plus accounts can generate 100+ 2K images in five hours ([Tutorial](https://ziy68cvfvu3.feishu.cn/wiki/LDSOwPzkhiNonkkNTF1ct2VBnNc)) (Based on official OpenAI OAuth PKCE authorization flow, non-reverse engineered).
...
- **[2026-04-23]**: Supported the gpt-image-2 model. Meanwhile, the export effect for editable backgrounds has been improved due to model capability upgrades (Select "Generative Acquisition" in Settings - Export Options - Background Acquisition).
...
Have you ever found yourself in this predicament: a presentation is due tomorrow, but your slides are still blank; your mind is filled with brilliant ideas, but all your enthusiasm is drained by tedious layout and design tasks?
...
These flaws make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs: "speed" and "beauty." Even those claiming to be "Vibe PPTs" are, in my eyes, still far from being truly "Vibe."
...
1. **Beginners**: Generate beautiful PPTs quickly with zero entry barrier and no design experience, eliminating the hassle of choosing templates.
2. **PPT Professionals**: Gain quick design inspiration by referencing AI-generated layouts and combinations of graphic and text elements.
3. **Educators**: Rapidly convert teaching content into illustrated lesson plan PPTs to enhance classroom effectiveness.
4. **Students**: Complete class presentations quickly, allowing them to focus their energy on content rather than layout and formatting.
...
Supports three starting modes—**Idea**, **Outline**, and **Page Description**—to suit different creative workflows.
...
- **Natural Language Editing**: Supports modifying outlines or descriptions using natural language prompts (e.g., "Change page three to a case study"), with the AI responding and adjusting in real-time.
...
- **Automatic Image Storage**: Images parsed from documents will be automatically added to the project's asset library once the reference file is associated with the project, allowing for future reuse.
...
- **Local Redrawing**: Make conversational modifications to specific areas (e.g., "Change this chart to a pie chart").
...
- **Playback Settings**: Enable page transition animations before exporting to PPTX, supporting classic effects like Fade.
...
| Re-editing | Prompt modification | **Selection editing + Oral editing** |
...
| 🔄 In Progress | Support for exporting editable .pptx with multi-layered, precise cutouts |
| 🔄 In Progress | Web Search |
| 🔄 In Progress | Agent Mode |
| ✅ Completed | TTS narrated video export (Multi-voice in Chinese/English/Japanese, subtitles) |
...
1. One-click deploy and start this application via Rainyun (High bandwidth, ideal for HD image generation and downloading. New users get a free trial)
...
**(Optional, you can also configure this in the user interface after startup; [click here for the tutorial](https://ziy68cvfvu3.feishu.cn/wiki/GiNawdmpiinSRqkGspocqEWAnkh?from=from_copylink))** Edit the `.env` file to configure the necessary environment variables:

<details>
<summary>Click to expand details</summary>
  
> **The LLM interfaces in this project follow the AIHubMix platform format standard. It is recommended to use [AIHubMix (Click here to visit)](https://api.inferera.com/?aff=17EC) to obtain API keys and reduce migration costs.**<br>
...
# SenseTime SenseNova U1 Image Model (Retaining legacy Provider, images via OpenAI-compatible path)
...
# Note: Agent Plan requires an exclusive API Key and model names (doubao-seed-2.1-turbo / doubao-seedream-5.0-lite)
...
**Use the new editable export configuration method to achieve better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (sufficient free usage quota is available). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.
...
After starting, you can navigate to **Settings → About → Check for Updates** within the application. The app will determine if updates are available based on the current version SHA; when running from source, the current Git SHA will also be used for comparison.
...
# Then, following the method described earlier, open and edit the `.env` file to configure your API key

# Project Title

## Overview
This is an open-source project designed to provide an efficient and professional solution for your technical needs. It aims to offer a robust architecture while maintaining simplicity and ease of use.

## Key Features
- **High Performance**: Optimized for speed and resource efficiency to handle demanding tasks.
- **Easy Integration**: Seamlessly integrates with existing workflows and third-party services.
- **User-Friendly**: Provides an intuitive configuration process and clear documentation for developers of all levels.
- **Extensible**: Highly modular design that allows for custom plugins and extensions.
...
The frontend will automatically connect to the backend service specified by `BACKEND_PORT` (defaulting to `http://localhost:5011`) via Vite proxy. To modify this, please set `BACKEND_PORT` in the `.env` file at the project root.
...
Open your browser and visit it to use the application.
