...
def get_quality_enhancement_prompt() -> str:
    """
    生成画质提升的 prompt
    用于在元素抹除后，使用生成式模型修复抹除痕迹并提升整体画质
    
    注意：调用时会传入原图作为参考（additional_ref_images），让模型可以对比找到需要修复的区域
    """
    prompt = """\
你是一位专业的图像修复专家。

我给你两张图片：
1. **第一张是待修复图**（当前图）：经过文字/对象抹除操作后的图片
2. **第二张是原图**（参考图）：包含文字和各种元素的原始图片

待修复图中，抹除工具在原本有文字的区域留下了一些明显的修复痕迹，包括：
- 色块不均匀、颜色不连贯
- 模糊的斑块或涂抹痕迹
- 与周围背景不协调的区域
- 可能的纹理断裂或图案不连续
...
你的任务：
1. **对比两张图片**，找出原图中有文字/对象但待修复图中已被抹除的区域
2. **修复这些区域**，让背景看起来自然、连贯，像从未有过任何元素一样
3. **保持其他区域不变**，不要改动没有被抹除的部分

要求：
- 让被修复的区域与周围背景完美融合，保持纹理、颜色、图案的连续性
- 提升整体画质，消除模糊、噪点、伪影
- **不要恢复被删除的文字或对象**，目标是干净的背景
- 保持图片的原始构图、布局、色调风格
- 输出图片的尺寸必须与待修复图一致
