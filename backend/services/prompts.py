...
1. 配合关键词介绍的内容要简洁精炼，每条要点控制在15-25字以内
2. 条理清晰，使用列表形式组织内容
3. 避免冗长的句子和复杂的表述
4. 确保内容可读性强，适合在演示时展示
5. 不要包含任何额外的说明性文字或注释
6. 页面素材主要是为了给设计师提供参考，请尽量详细描述新设计的图片内容和风格要求，快速还原一张真实世界的效果,以便设计师表达需求
7. 为了丰富性,在适当页面,如果你觉得 1 个元素单薄,可以加入 1-2 个页面图片元素.
...
页面素材1: 
[一张写实风格的夜景铺装互动效果图。画面取景于天津滨江道人行街道，地面铺装以深浅变化的高渗透材料为基底，局部嵌入感应发光地砖。行人行走时，脚下产生柔和扩散的光影涟漪，引导行进方向。夜间灯光克制不刺眼，与沿街商业照明形成层次。铺装表面可见细密防滑纹理，雨后地面无明显积水，整体呈现安全、趣味与科技感并存的街道步行体验。]
{get_language_instruction(language)}
""")
# 其他页面素材（如果有请积极添加，包括markdown图片链接、公式、表格等）【关于图片】如果参考文件中包含以 /files/ 开头的本地文件URL图片（例如 /files/mineru/xxx/image.png），请将这些图片以markdown格式输出，例如：![图片描述](/files/mineru/xxx/image.png)。这些图片会被包含在PPT页面中。   
    final_prompt = files_xml + prompt
...
<design_guidelines>
- 要求中文文字清晰锐利, 打印海报级别,16:9比例。
- 如果提供了模板图片，请在整体配色和设计语言上尽量保持与模板风格一致；
  如果没有模板图片，则根据页面内容设计统一、专业且美观的配色和设计语言。
- 根据内容自动设计最完美的构图，不重不漏地渲染"页面描述"中的文本。
- 如非必要，禁止出现 markdown 格式符号（如 # 和 * 等）。
- 如果存在模板图片，只参考其风格设计，禁止出现模板中的文字。
