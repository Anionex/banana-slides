...
def _settings_row_to_runtime_override(settings_row) -> Dict[str, object]:
    return {
        ...
        "BAIDU_OCR_API_KEY": settings_row.baidu_ocr_api_key,
    }
...
def build_effective_settings_override(user_id: str | None = None) -> Dict[str, object]:
    ...
    if not user_id:
        logger.info("Building runtime settings override from global Settings only")
        return _settings_row_to_runtime_override(global_settings)
