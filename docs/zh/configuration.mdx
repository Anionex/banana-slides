...
桌面版（PyInstaller 打包）会显式收集并注册 LazyLLM 的全部在线厂商（qwen、doubao、deepseek、glm、kimi、minimax、sensenova、siliconflow、ppio、aiping、openai）。LazyLLM 通过 `pkgutil.iter_modules` 动态发现供应商，在打包环境中该机制可能失效导致 `Unsupported source: xxx`；Banana Slides 已在启动时显式导入全部供应商模块，并会把配置的厂商名解析为 LazyLLM 实际注册名，无需额外配置。
