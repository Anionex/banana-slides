...
使用 OpenAI 兼容的图片模型时，可在「设置 → 图像生成模型 → 图片质量档位」选择生成质量。该选项只在图片提供商为 OpenAI / 火山 Agent Plan 时显示，其他提供商不受影响。
...
- 也可以通过环境变量设置默认值：`IMAGE_QUALITY=high`（可选 `auto` / `low` / `medium` / `high` / `xhigh` / `max`），设置页里的值优先级更高。
