...
| `apimart` | APIMart 文本、图片理解与异步图片生成 API |
...
选择设置页中的 **APIMart** 胶囊或推荐方案卡片后，会自动填写 APIMart 端点与推荐模型：

- 文本模型：`gpt-5`
- 图片描述模型：`gpt-4o`
- 图片模型：`gpt-image-2`

APIMart 图片生成采用异步任务。Banana Slides 会自动提交任务、轮询状态并下载结果，用户不需要手动处理任务 ID。
...
API Key 注册入口见项目 README 顶部的 APIMart 赞助商信息。
