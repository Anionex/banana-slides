...
## What should I do if a project stays on “Loading project”?

Project editor routes now stop the loading animation after a failed request and show the specific backend error. You can:

1. Click **Retry** to load the same project in place after a temporary backend or network interruption.
2. Click **Back to Home** if the project was deleted or you do not want to continue troubleshooting. When the project no longer exists, the app also clears the stale current-project record from the browser.

This recovery screen is available in the Outline Editor, Description Editor, Per-page Template Setup, and Slide Preview routes.
