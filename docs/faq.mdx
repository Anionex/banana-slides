...
## PPT Renovation says the MinerU token is expired, invalid, or unauthorized

The PDF parsing service cannot use the current MinerU credential. Update the token in **Settings → MinerU Configuration**, then run the **MinerU PDF Parsing** service test. After it passes, create the renovation project again. For deployment, configure `MINERU_TOKEN` as described in [Configuration](/configuration#mineru-pdf-parsing).
