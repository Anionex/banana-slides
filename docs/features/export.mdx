...
Export task history is scoped by project. Clicking **Clear** in one project removes only that project's completed or failed tasks and leaves export history from other projects untouched. After a refresh, if a restored export task is no longer available on the backend, the panel marks it as failed and asks you to export again instead of leaving it stuck as in progress.
