...
Once the outline looks good, click **Next →** in the top-right to go to the [Description Editor](/features/descriptions). If you just changed the original idea or outline in the left panel, Banana Slides saves it before navigating. When that save fails, the editor stays on the current page and shows an error instead of silently continuing with unsaved content.
