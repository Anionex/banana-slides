...
- If you do not have a reference image yet, click **Generate 5 template candidates**. The system asynchronously generates 5 temporary template-style images; select one when generation completes. The selected image continues through the existing template upload flow and becomes the style reference for downstream page generation.

<Note>
  Template candidates are transient previews. They are not saved to My Templates or candidate history. Generation shows progress; if it fails, adjust the style description or image model settings and try again.
</Note>
