...
Imports, material management, help, and confirmation screens built on the shared dialog component can be used without a mouse:

- Focus automatically moves into the active dialog when it opens
- `Tab` and `Shift+Tab` cycle within the dialog instead of reaching the background page
- `Esc` closes the dialog and returns focus to the button that opened it
- Dialog titles and close buttons expose localized screen-reader names
