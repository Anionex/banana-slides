...
| `apimart` | APIMart text, vision, and asynchronous image-generation APIs |
...
Selecting the **APIMart** pill or recommendation card in Settings fills the APIMart endpoint and recommended models:

- Text model: `gpt-5`
- Image-caption model: `gpt-4o`
- Image model: `gpt-image-2`

APIMart image generation uses asynchronous tasks. Banana Slides submits the task, polls its status, and downloads the result automatically; users do not need to handle task IDs.
...
See the APIMart sponsor entry at the top of the project README for the API-key sign-up link.
