...
Selecting **APIMart** in Settings fills the API endpoint and the recommended text, vision, and image-generation models. You can also configure it through `.env`:
...
Banana Slides submits and polls image-generation tasks automatically, so users do not need to handle task IDs.
