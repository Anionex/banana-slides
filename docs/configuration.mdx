...
When the image model is served over an OpenAI-compatible API, pick the generation quality under Settings → Image Generation Model → Image Quality. The control only appears when the image provider is OpenAI or Volcengine Agent Plan; other providers are unaffected.
...
- You can set the default through the environment instead: `IMAGE_QUALITY=high` (one of `auto` / `low` / `medium` / `high` / `xhigh` / `max`). The value saved in Settings takes precedence.
