...
To get an API key, open AIHubMix and sign in or create an account. Go to **Console**, choose **Develop → API Keys** in the left sidebar, click **Add key**, then copy the generated key into the Settings page or `.env`. Add credits in **Console → Top Up** before making requests.
