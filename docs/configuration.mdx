...
[AIHubMix](https://aihubmix.com/?aff=17EC) is a recommended API proxy that supports multiple model providers through a single OpenAI-compatible endpoint. This simplifies configuration — you only need one API key for access to Gemini, GPT, Claude, and more.
...
[Apply for an AIHubMix API key here](https://aihubmix.com/token?aff=17EC).

## MinerU (PDF Parsing)

[MinerU](https://mineru.net) provides high-quality PDF parsing for reference file uploads. Configure it to enable PDF content extraction:
...
[Apply for a MinerU token here](https://mineru.net/apiManage/token).

## Baidu API Key

For enhanced editable PPTX export with OCR-based text extraction, obtain an API key from [Baidu Cloud Console](https://console.bce.baidu.com/iam/#/iam/apikey/list) (generous free tier available):
