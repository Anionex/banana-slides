...
`--order-index` must be a non-negative integer. Banana Slides rejects invalid
page positions before saving so API and CLI callers get a clear error instead
of an uncertain page order.
