...
test.describe('Editable export failure UI', () => {
  test('sends editable PPTX text-only option only when checked', async ({ page }) => {
    const projectId = 'mock-editable-export-text-only';
    ...
    await page.route(url => new URL(url).pathname.startsWith('/api/'), async route => {
      ...
      if (url.pathname === `/api/projects/${projectId}/export/editable-pptx`) {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: { task_id: `editable-export-task-${requestBodies.length}` },
          }),
        });
      }

      if (url.pathname.startsWith(`/api/projects/${projectId}/tasks/editable-export-task-`)) {
        return route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            success: true,
            data: {
              task_id: url.pathname.split('/').pop(),
              task_type: 'EXPORT_EDITABLE_PPTX',
              status: 'COMPLETED',
              progress: { percent: 100, download_url: '/files/mock/text-only.pptx' },
            },
          }),
        });
      }
    ...
    const textOnlyCheckbox = page.getByRole('checkbox', { name: /仅文字层可编辑/ });
    await expect(textOnlyCheckbox).not.toBeChecked();
    await page.getByRole('button', { name: '开始导出' }).click();

    await expect.poll(() => requestBodies.length, { timeout: 10000 }).toBe(1);
    expect(requestBodies[0].text_only).toBe(false);

    await page.locator('button:has-text("导出")').first().click();
    await page.getByRole('button', { name: /导出可编辑 PPTX/ }).click();
    await expect(textOnlyCheckbox).not.toBeChecked();
    await textOnlyCheckbox.check();
    await page.getByRole('button', { name: '开始导出' }).click();

    await expect.poll(() => requestBodies.length, { timeout: 10000 }).toBe(2);
    expect(requestBodies[1].text_only).toBe(true);
  });
  ...
  test('shows normalized task panel error when style extraction fails', async ({ page }) => {
    ...
    const exportTasksButton = page.getByLabel('导出任务');
    await exportTasksButton.click();
    await expect(page.getByText('当前图片样式提取模型不支持图片输入')).toBeVisible({ timeout: 10000 });
    await expect(exportTasksButton).toBeVisible({ timeout: 10000 });
  });
