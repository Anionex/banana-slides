...
import { authApi, logoutUser } from '../../api/auth';
...
export default function ResetPasswordPage() {
  ...
  const [checking, setChecking] = useState(isSupabaseConfigured() && !legacyToken);
  const [hasRecoveryContext, setHasRecoveryContext] = useState(!isSupabaseConfigured() ? Boolean(legacyToken) : Boolean(legacyToken));

  useEffect(() => {
    let active = true;

    const validateRecoveryContext = async () => {
      if (!isSupabaseConfigured() || legacyToken) {
        setChecking(false);
        setHasRecoveryContext(Boolean(legacyToken));
        return;
      }
      ...
      try {
        ...
        setHasRecoveryContext(Boolean(data.session));
      } catch {
        if (active) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.resetPassword(legacyToken, password);
      setSuccess(true);
      setTimeout(async () => {
        await logoutUser();
      ...
      const message = errorData?.error?.message || errorData?.message || err?.message || t('resetPassword.resetFailed');
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-background-primary">
        <div className="text-center">
          ...
          <div className="text-gray-600 dark:text-foreground-secondary">{t('resetPassword.checking')}</div>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-md relative z-10 px-6">
        ...
        <div className="bg-white dark:bg-background-elevated rounded-2xl p-8 shadow-xl border border-gray-200 dark:border-border-primary">
          {success ? (
            ...
            <>
              <div className="text-center mb-8">
                <h1 className="text-2xl font-semibold text-gray-900 dark:text-foreground-primary mb-2">{t('resetPassword.title')}</h1>
                <p className="text-gray-600 dark:text-foreground-secondary">{t('resetPassword.description')}</p>
              </div>
