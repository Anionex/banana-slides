...
import { getSettings, getTestStatus } from '@/api/endpoints';
...
export function PublicSettings() {
  ...
  const save = async () => {
    setSaving(true);
    const fields = ['partner', 'image_resolution', 'image_aspect_ratio', 'max_description_workers', 'max_image_workers',
  ...
  const reset = () => confirm('将清空你保存的所有合作方密钥及个人配置，其他访客不受影响。', () => { void performReset(); }, { title: '重置个人设置' });
  ...
  const test = async (name: string) => {
    setTesting(name);
    try {
      const response = (await apiClient.post(`/api/settings/tests/${name}`, {})).data;
      ...
      while (Date.now() < deadline) {
        ...
        if (status.data?.status === 'COMPLETED') {
          const result = status.data.result as { success?: boolean; message?: string } | undefined;
          show({ message: result?.message || '测试完成', type: result?.success === false ? 'error' : 'success' }); return;
        }
        if (status.data?.status === 'FAILED') throw new Error(status.data.error || '服务测试失败');
        await new Promise(resolve => setTimeout(resolve, 1500));
      }
      throw new Error('测试仍在运行，请稍后重试。');
    } catch (e) { show({ message: e instanceof Error ? e.message : '服务测试失败', type: 'error' }); }
    finally { setTesting(''); }
  };
    ...
    <main className="max-w-3xl mx-auto p-4 md:p-8 space-y-5">
      ...
      {loading ? <p role="status">正在加载个人设置…</p> : error ? <div role="alert">{error}<Button onClick={() => void load()}>重试</Button></div> : <>
        <section className={sectionClass} aria-label="API 配置">
          ...
          <label className="block text-sm space-y-2"><span>合作方配置</span>
            <select className={inputClass} value={String(data.partner)} onChange={e => setData(prev => ({ ...prev, partner: e.target.value, api_key: '' }))}>
              {Object.entries(publicPartners).map(([id, profile]) => <option key={id} value={id}>{profile.name}</option>)}
            </select>
          </label>
          <p className="text-sm text-gray-500">{partner?.key_hint}。切换合作方后使用各自保存的密钥。</p>
          <label className="block text-sm space-y-2"><span>API Key</span><input type="password" autoComplete="new-password" className={inputClass} value={String(data.api_key || '')} onChange={e => set('api_key', e.target.value)} placeholder={data.partner === saved.partner && Number(saved.api_key_length) > 0 ? '已保存，留空保持不变' : '输入该合作方的 API Key'} /></label>
        ...
        <section className={sectionClass} aria-label="模型配置"><h2 className="font-semibold">模型配置</h2>
          ...
          <p className="text-xs text-gray-500">模型由合作方配置固定。</p>
        </section>
        <section className={sectionClass}><h2 className="font-semibold">图像与生成设置</h2>
          {([['image_resolution', '图像清晰度', ['1K', '2K', '4K']], ['image_aspect_ratio', '默认图像比例', ['16:9', '4:3', '1:1']], ['output_language', '输出语言', ['zh', 'en', 'ja', 'auto']], ['description_generation_mode', '描述生成模式', ['streaming', 'parallel']]] as const).map(([key, label, choices]) => <label className="block text-sm space-y-2" key={key}><span>{label}</span><select className={inputClass} value={String(data[key])} onChange={e => set(key, e.target.value)}>{choices.map(value => <option key={value} value={value}>{({ zh: '中文', en: 'English', ja: '日本語', auto: '自动', streaming: '流式', parallel: '并行' } as Record<string, string>)[value] || value}</option>)}</select></label>)}
        ...
        <section className={sectionClass}><h2 className="font-semibold">推理预算</h2>
          {([['text_thinking_budget', '文本推理预算', 'enable_text_reasoning'], ['image_thinking_budget', '图像推理预算', 'enable_image_reasoning']] as const).map(([key, label, enabled]) => <label className="block text-sm space-y-2" key={key}><span>{label}</span><input className={inputClass} type="number" min={0} max={32768} disabled={!data[enabled]} value={Number(data[key])} onChange={e => set(key, Number(e.target.value))} /></label>)}
        </section>
        ...
        <div className="flex gap-3"><Button onClick={() => void save()} disabled={saving || !!testing}>{saving ? '正在保存…' : '保存设置'}</Button><Button variant="secondary" onClick={() => void reset()} disabled={saving || !!testing}>重置设置</Button></div>
        <section className={sectionClass}><h2 className="font-semibold">服务测试</h2><p className="text-sm text-gray-500">请先保存设置。测试会调用对应服务；图像或解析测试可能需要 30–60 秒。</p>
          <div className="flex flex-wrap gap-2">{[['text-model', '文本模型'], ['caption-model', '图片识别'], ['image-model', '图像生成'], ['mineru-pdf', 'MinerU 解析']].map(([name, label]) => <Button key={name} variant="secondary" disabled={!!testing || saving} onClick={() => void test(name)}>{testing === name ? '正在测试…' : `测试${label}`}</Button>)}</div>
          {testing && <p role="status">测试正在运行，请稍候…</p>}
        </section>
      </>}
