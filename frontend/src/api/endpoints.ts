...
export const generateOutlineStream = async (
  ...
  const response = await authenticatedFetch(`/api/projects/${projectId}/generate/outline/stream`, {
    method: 'POST',
    headers: {
...
export const generateDescriptionsStream = async (
  ...
  const response = await authenticatedFetch(`/api/projects/${projectId}/generate/descriptions/stream`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(accessCode ? { 'X-Access-Code': accessCode } : {}),
