...
const projectSettingsI18n = {
  zh: {
    projectSettings: {
      ...
      iconSubjectExtraction: "图标透明背景（百度智能抠图）",
      iconSubjectExtractionDesc: "开启后，导出时对识别为小尺寸图标的图片调用百度智能抠图 API，获取透明背景的 PNG 后再贴入 PPT，避免图标自带的原 PPT 背景在新底色上突兀显示。仅对小尺寸图标生效，照片/插图保持原矩形裁剪。",
      iconSubjectExtractionTip: "需要在「全局设置」中配置 BAIDU_API_KEY；未配置或单张图标处理失败时会自动回退到原矩形裁剪，不影响导出。",
  ...
  en: {
    projectSettings: {
      ...
      iconSubjectExtraction: "Icon Transparent Background (Baidu Subject Extraction)",
      iconSubjectExtractionDesc: "When enabled, small icon images detected during export will be processed via Baidu's subject extraction API to produce transparent-background PNGs before being placed in the PPT, avoiding the icon's original background clashing with the new slide background. Only applies to small icons; photos and illustrations keep the original rectangular crop.",
      iconSubjectExtractionTip: "Requires BAIDU_API_KEY configured in Global Settings. If the key is missing or extraction fails for an individual icon, it falls back silently to the original rectangular crop without affecting export.",
