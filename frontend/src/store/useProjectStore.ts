...
import { debounce, normalizeProject, normalizeErrorMessage, downloadFromUrl } from '@/utils';
  ...
  return {
          ...
          if (task.status === 'COMPLETED') {
            ...
            if (task.task_type === 'EXPORT_EDITABLE_PPTX' && task.progress) {
              ...
              if (downloadUrl) {
                ...
                setTimeout(() => {
                  downloadFromUrl(downloadUrl).catch(err => {
                    console.error('[导出可编辑PPTX] 下载失败:', err);
                  });
                }, 500);
              } else {
    ...
    exportPPTX: async (pageIds?: string[]) => {
      ...
      try {
        ...
        // 使用下载工具函数，支持 Electron 和浏览器
        await downloadFromUrl(downloadUrl);
      } catch (error: any) {
